...
class MonoFileMiniBar(QtWidgets.QWidget):
    ...
    def _show_handle_context_menu(self, pos):
        ...
        try:
            ...
            version_action = menu.addAction("ℹ️ v2.0.0")
    ...
    def _open_manager(self):
        ...
        if hasattr(self.manager, 'project_cb') and self.manager.project_cb.currentText().strip():
            ...
            print(f"💾 Saved current project: {project}")
    ...
    def _load_tabs_for_type(self, type_index):
        ...
        try:
            print(f"🔄 Loading tabs for type {type_index} ({'Assets' if type_index == 0 else 'Shots'})")
            ...
            if type_index == 0:  # Assets
                ...
                print(f"✅ Loaded {len(assets_configs)} asset tabs: {[c['name'] for c in assets_configs]}")
            else:  # Shots
                ...
                for conf in shots_configs:
                    ...
                    self.tab_cb.addItem(name, conf)
                print(f"✅ Loaded {len(shots_configs)} shot tabs: {[c['name'] for c in shots_configs]}")
            ...
            if saved_tab:
                ...
                if idx >= 0:
                    ...
                    print(f"✅ Restored tab: {saved_tab}")
                else:
                    self.tab_cb.setCurrentIndex(0)
                    print(f"⚠️ Saved tab '{saved_tab}' not found, using default")
            else:
                # Set default selection
                self.tab_cb.setCurrentIndex(0)
                print(f"✅ Using default tab selection")
            
            self.tab_cb.blockSignals(False)
            
            # Don't trigger refresh here - let the caller handle it
            print(f"✅ Tab dropdown now has {self.tab_cb.count()} items")
            
            # Don't trigger initial load here - let user interaction trigger it
                
        except Exception as e:
            print(f"⚠️ Error loading tabs for type: {e}")
    ...
    def _refresh_files_for_current_tab(self):
        ...
        try:
            if not self.manager:
                print("⚠️ No manager available for file refresh")
            ...
            if not tab_config:
                print("⚠️ No tab config available")
                return
            ...
            if not root or not project:
                print(f"⚠️ Missing root or project: root='{root}', project='{project}'")
                return
            
            print(f"🔄 Refreshing files for type {type_idx} ({'Assets' if type_idx == 0 else 'Shots'})")
            print(f"📍 Root: {root}")
            print(f"📍 Project: {project}")
            print(f"📍 Tab: {tab_config.get('name', 'Unknown')}")
            ...
            if type_idx == 0:  # Assets mode
                ...
                print(f"📍 Asset filters: type='{asset_type}', department='{department}', asset_name='{asset_name}'")
                ...
                if asset_files:
                    ...
                    if len(asset_files) > 5:
                        print(f"  ... and {len(asset_files) - 5} more asset files")
                ...
                print(f"📍 Shot target dir: {target_dir}")
                
                if not os.path.isdir(target_dir):
                    print(f"⚠️ Target directory does not exist: {target_dir}")
                    return
                ...
                print(f"📍 Found {len(files)} shot files")
                if files:
                    print("Shot files details:")
                    for i, filepath in enumerate(files[:5]):
                        print(f"  {i+1}. {os.path.basename(filepath)}")
                    if len(files) > 5:
                        print(f"  ... and {len(files) - 5} more shot files")
                self.populate(files)
    ...
    def _refresh_files_standalone(self):
        ...
        try:
            ...
            if not root or not project:
                print(f"⚠️ Missing saved settings: root='{root}', project='{project}'")
            ...
            print(f"⚠️ Error in standalone refresh: {e}")
    ...
    def _load_minibar_settings(self):
        ...
        try:
            ...
            print(f"⚠️ Error loading minibar settings: {e}")
    
    def _apply_ui_scale(self):
        """Apply UI scale to MiniBar"""
        try:
            scale = self.s.value("minibar_ui_scale", 100, type=int)
            print(f"🔍 Loading UI scale setting: {scale}%")
            ...
            if scale != 100:
                ...
                self.setFont(font)
                print(f"🔍 Font size: {base_size} → {new_font_size}")
                
                # Scale button sizes
                base_size = 24
                scaled_size = int(base_size * scale_factor)
                print(f"🔍 Button size: {base_size} → {scaled_size}")
                ...
                print(f"🔍 Type/Dept button size: 70 → {type_width}")
                
                # Scale handle
                handle_width = int(20 * scale_factor)
                self.handle_area.setFixedWidth(handle_width)
                print(f"🔍 Handle width: 20 → {handle_width}")
                ...
                print(f"🔍 File display width: 160 → {display_width}")
                ...
                print(f"🔍 Layout margins: (4,3,6,3) → ({margin_left},{margin_top},{margin_right},{margin_bottom})")
                print(f"🔍 Layout spacing: 3 → {spacing}")
                
                print(f"✅ UI scale applied successfully: {scale}%")
            else:
                print(f"🔍 No scaling needed: {scale}%")
                
        except Exception as e:
            print(f"⚠️ Error applying UI scale: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _open_settings(self):
        ...
        try:
            ...
            print(f"⚠️ Error opening settings dialog: {e}")
    ...
    def _on_settings_changed(self):
        ...
        try:
            ...
            print(f"⚠️ Error handling settings change: {e}")
