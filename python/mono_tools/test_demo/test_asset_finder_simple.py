#!/usr/bin/env python3
"""
Simple test script for Asset Finder functionality
Tests only the core functions without Houdini dependencies
"""
...
def test_asset_helpers_direct():
    """Test the asset helper functions directly without importing the full package"""
    print("\nTesting asset helper functions...")
    
    try:
        # Import only the specific module
        file_manager_path = os.path.join(project_root, 'python', 'mono_tools', 'file_manager')
        sys.path.insert(0, file_manager_path)
        ...
        import file_manager_helpers
        collect_asset_files = file_manager_helpers.collect_asset_files
        list_asset_types = file_manager_helpers.list_asset_types
        list_asset_names = file_manager_helpers.list_asset_names
        list_departments = file_manager_helpers.list_departments
        infer_asset_name = file_manager_helpers.infer_asset_name
        infer_department = file_manager_helpers.infer_department
...
def test_asset_models_direct():
    """Test the asset table model directly"""
    ...
    try:
        ...
        class MockQtGui:
            class QStandardItemModel:
                ...
                def add_row(self, asset, dept, ver, name, ext, folder, mtime, sz, fullpath):
                    self._rows.append((asset, dept, ver, name, ext, folder, mtime, sz, fullpath))
        ...
        import file_manager_models
        AssetTableModel = file_manager_models.AssetTableModel
        ...
        headers = [model.headerData(i, MockQtCore.Qt.Horizontal, MockQtCore.Qt.DisplayRole) for i in range(model.columnCount())]
...
def main():
    ...
    # Test asset helpers
    if not test_asset_helpers_direct():
        success = False
    
    # Test asset models
    if not test_asset_models_direct():
        success = False
